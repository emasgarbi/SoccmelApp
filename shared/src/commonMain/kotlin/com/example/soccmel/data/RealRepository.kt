...
object RealRepository {
    ...
    init {
        ...
        globalPolls.add(Poll(
            ...
            creatorId = u1.id, 
            creatorName = u1.name, 
            options = listOf(PollOption(id = randomUUID(), text = "Barazzo"), PollOption(id = randomUUID(), text = "Mutenye"), PollOption(id = randomUUID(), text = "Alce Nero")),
            comments = listOf(
                Comment(id = randomUUID(), userId = u2.id, userName = u2.name, text = "Il Barazzo ha sempre i vini migliori!", timestamp = getTimeMillis())
            ),
        ...
        globalPolls.add(Poll(
            ...
            creatorId = u2.id, 
            creatorName = u2.name, 
            options = listOf(PollOption(id = randomUUID(), text = "Meloncello (classico)"), PollOption(id = randomUUID(), text = "Casaglia (pro)")),
            comments = emptyList(),
            timestamp = getTimeMillis()
        ))

        globalEvents.add(BolognaEvent(id = randomUUID(), title = "Mercato Ritrovato", description = "Il mercato dei produttori locali alla Cineteca.", date = "Ogni Sabato mattina", location = "Piazzetta Pasolini", category = "Cibo", isPublic = true, isFixed = true, supportContact = "info@mercatocritico.it"))
        globalEvents.add(BolognaEvent(id = randomUUID(), title = "Cinema Sotto le Stelle", description = "Proiezioni gratuite in Piazza Maggiore.", date = "Giugno - Agosto", location = "Piazza Maggiore", category = "Cultura", isPublic = true, isFixed = true, supportContact = "cineteca@comune.bologna.it"))
        globalEvents.add(BolognaEvent(id = randomUUID(), title = "Cioccoshow", description = "La festa del cioccolato artigianale nel cuore della città.", date = "Novembre (Annuale)", location = "Piazza XX Settembre", category = "Cibo", isPublic = true, isFixed = true))
        globalEvents.add(BolognaEvent(id = randomUUID(), title = "Fiera di Santa Lucia", description = "I mercatini di Natale sotto il portico della Chiesa dei Servi.", date = "Novembre - Dicembre", location = "Strada Maggiore", category = "Cultura", isPublic = true, isFixed = true))
        globalEvents.add(BolognaEvent(id = randomUUID(), title = "Workshop Fotografia Urbana", description = "Impariamo a fotografare i portici insieme.", date = "Sabato 15 Marzo ore 15:00", location = "Piazza Santo Stefano", category = "Cultura", isPublic = true, isFixed = false, supportContact = "info@bolognafotografia.it"))
        globalEvents.add(BolognaEvent(id = randomUUID(), title = "Calcetto ai Giardini Margherita", description = "Ci manca un portiere, chi viene?", date = "Stasera ore 19:00", location = "Giardini Margherita", category = "Sport", isPublic = false, organizerId = u1.id, participantIds = listOf(u1.id)))
        globalEvents.add(BolognaEvent(id = randomUUID(), title = "Aperitivo in via del Pratello", description = "Ritrovo per un bicchiere di vino.", date = "Venerdì ore 18:30", location = "Via del Pratello", category = "Social", isPublic = false, organizerId = u2.id, participantIds = listOf(u2.id)))
    ...
    fun init() {
        ...
        val savedVersion = s.getInt(KEY_STORAGE_VERSION, 0)
        if (savedVersion < CURRENT_VERSION) {
            s.remove(KEY_ACCOUNTS)
            s.remove(KEY_USER_ID)
            s.remove(KEY_USER_NAME)
            s.remove(KEY_USER_EMAIL)
            s.putInt(KEY_STORAGE_VERSION, CURRENT_VERSION)
        }

        val savedAccounts = s.getString(KEY_ACCOUNTS, "")
        savedAccounts.split(",").filter { it.isNotBlank() }.forEach { entry ->
            ...
            if (parts.size >= 2) {
                ...
                val id = if (parts.size >= 3 && parts[2].isNotBlank()) parts[2] else randomUUID()
                val name = if (parts.size >= 4 && parts[3].isNotBlank()) parts[3] else email.substringBefore("@")
                
                globalPasswords[email] = storedPass
                if (!globalUsers.containsKey(id)) {
                    globalUsers[id] = User(id = id, name = name, email = email, bio = "Cittadino di Bologna", joinDate = "Marzo 2026", interests = listOf("🏰 Bologna"))
                }
            }
        }

        val id = s.getStringOrNull(KEY_USER_ID)
        if (id != null) {
    ...
    fun login(email: String, password: String): String? {
        ...
        if (savedPassword == null) return "Account non trovato. Registrati per iniziare."
        if (savedPassword.trim() != normalizedPassword) return "Password errata per questo account"
        
        val user = globalUsers.values.find { it.email.lowercase() == normalizedEmail } ?: run {
            val id = randomUUID()
            val u = User(id = id, name = normalizedEmail.substringBefore("@"), email = normalizedEmail, bio = "Cittadino di Bologna", joinDate = "Marzo 2026")
            globalUsers[id] = u
            u
        }

        s.putString(KEY_USER_ID, user.id)
        s.putString(KEY_USER_NAME, user.name)
        s.putString(KEY_USER_EMAIL, user.email)
        _currentUser.value = user
        refreshPolls()
        refreshEvents()
        refreshFriendRequests()
    ...
    fun register(email: String, password: String): String? {
        ...
        if (globalPasswords.containsKey(normalizedEmail)) return "L'email è già registrata. Effettua il login."
        ...
        val user = User(id = userId, name = name, email = normalizedEmail, bio = "Cittadino di Bologna pronto a contribuire!", joinDate = "Marzo 2026", interests = listOf("🏰 Bologna"))
        
        globalUsers[userId] = user
        globalPasswords[normalizedEmail] = normalizedPassword
        
        val currentSaved = s.getString(KEY_ACCOUNTS, "")
        val newEntry = "$normalizedEmail|$normalizedPassword|$userId|$name"
        val newSaved = if (currentSaved.isBlank()) newEntry else "$currentSaved,$newEntry"
        s.putString(KEY_ACCOUNTS, newSaved)
    ...
    private fun sendFriendRequestFrom(fromUserId: String, toUserId: String) {
        ...
        globalFriendRequests.add(FriendRequest(id = randomUUID(), fromUserId = fromUserId, fromUserName = fromUser.name, toUserId = toUserId, timestamp = getTimeMillis()))
    }
    ...
    fun sendFriendRequest(toUserId: String) {
        ...
        globalFriendRequests.add(FriendRequest(id = randomUUID(), fromUserId = user.id, fromUserName = user.name, toUserId = toUserId, timestamp = getTimeMillis()))
        refreshFriendRequests()
    }
    ...
    fun createPoll(question: String, options: List<String>) {
        val user = _currentUser.value ?: return
        val newPoll = Poll(id = randomUUID(), question = question, creatorId = user.id, creatorName = user.name, options = options.map { PollOption(id = randomUUID(), text = it) }, timestamp = getTimeMillis())
    ...
    fun createEvent(title: String, description: String, date: String, location: String, category: String, isPublic: Boolean) {
        val user = _currentUser.value ?: return
        val newEvent = BolognaEvent(id = randomUUID(), title = title, description = description, date = date, location = location, category = category, isPublic = isPublic, isFixed = false, organizerId = user.id, participantIds = listOf(user.id))
    ...
    fun addComment(pollId: String, text: String) {
        ...
        if (pollIndex != -1) {
            ...
            val newComment = Comment(id = randomUUID(), userId = user.id, userName = user.name, text = text, timestamp = getTimeMillis())
